...
VPinball::VPinball()
{
   ...
   std::cout << "Path: " << m_szMyPath << std::endl;
   std::cout << "Player: " << g_pplayer << std::endl;
...
///<returns>Handle to Event that get ack. If event is finished (unsure)</returns>
///</summary>
void VPinball::InitTools() { m_ToolCur = IDC_SELECT; }
...
void VPinball::AddMDITable(PinTableMDI *mdiTable) { AddMDIChild(mdiTable); }
...
void VPinball::SetActionCur(const string &szaction) { SendMessage(m_hwndStatusBar, SB_SETTEXT, 3 | 0, (size_t)szaction.c_str()); }

void VPinball::SetStatusBarElementInfo(const string &info) { SendMessage(m_hwndStatusBar, SB_SETTEXT, 4 | 0, (size_t)info.c_str()); }

bool VPinball::OpenFileDialog(const string &initDir, std::vector<std::string> &filename, const char *const fileFilter, const char *const defaultExt, const DWORD flags,
   const std::string &windowTitle) //!! use this all over the place and move to some standard header
{
   CFileDialog fileDlg(
...
bool VPinball::SaveFileDialog(const string &initDir, std::vector<std::string> &filename, const char *const fileFilter, const char *const defaultExt, const DWORD flags,
   const std::string &windowTitle) //!! use this all over the place and move to some standard header
{
   CFileDialog fileDlg(
      FALSE, defaultExt, initDir.c_str(), nullptr, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER | flags, fileFilter); // OFN_EXPLORER needed, otherwise GetNextPathName buggy
   if (!windowTitle.empty())
      fileDlg.SetTitle(windowTitle.c_str());
   if (fileDlg.DoModal(*this) == IDOK)
   {
...
void VPinball::SetPosCur(float x, float y)
{
   ...
   if (m_convertToUnit != 2)
   {
      ...
      {
      case 0: sprintf_s(szT, "%.2f, %.2f %s", ConvertToUnit(x), ConvertToUnit(y), " (inch)"); break;
      case 1: sprintf_s(szT, "%.2f, %.2f %s", ConvertToUnit(x), ConvertToUnit(y), " (mm)"); break;
      default: assert(!"wrong unit"); break;
      }
...
void VPinball::ClearObjectPosCur() { SendMessage(m_hwndStatusBar, SB_SETTEXT, 1 | 0, (size_t) ""); }

float VPinball::ConvertToUnit(const float value) const
{
   switch (m_convertToUnit)
   {
   case 0: return vpUnitsToInches(value);

   case 1: return vpUnitsToMillimeters(value);
...
void VPinball::PreCreate(CREATESTRUCT &cs)
{
   ...
   cs.dwExStyle = WS_EX_CLIENTEDGE | WS_EX_CONTROLPARENT // TAB key navigation
...
void VPinball::OnDestroy() { PostMessage(WM_QUIT, 0, 0); }
...
void VPinball::OnInitialUpdate()
{
   ...
   m_hwndStatusBar = CreateStatusWindow(WS_CHILD | WS_VISIBLE, "", GetHwnd(),
...
LRESULT VPinball::WndProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   case UWM_UPDATECOMMAND: return FinalWindowProc(uMsg, wParam, lParam);
   }
...
{
   if (g_pplayer)
      g_pplayer->m_ptable->PlaySound(bstr, 0, 1.f, 0.f, 0.f, 0, VARIANT_FALSE, VARIANT_TRUE, 0.f);
...
{
   if (g_pplayer)
      g_pplayer->m_ptable->FireKnocker(Count);
...
INT_PTR CALLBACK SecurityOptionsProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_INITDIALOG:
   {
      ...
      SetWindowPos(hwndDlg, nullptr, (rcMain.right + rcMain.left) / 2 - (rcDlg.right - rcDlg.left) / 2, (rcMain.bottom + rcMain.top) / 2 - (rcDlg.bottom - rcDlg.top) / 2, 0, 0,
         SWP_NOOWNERZORDER | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE /* | SWP_NOMOVE*/);
   ...
   case WM_CLOSE: EndDialog(hwndDlg, FALSE); break;
   }
...
INT_PTR CALLBACK FontManagerProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_COMMAND:
      ...
      case BN_CLICKED:
         switch (LOWORD(wParam))
         {
         case IDOK: EndDialog(hwndDlg, TRUE); break;
